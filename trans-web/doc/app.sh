...
# 根据实际情况来修改以下配置信息 ##################################
# JAVA应用程序的名称
APP_NAME="trans-serve-0.0.1-SNAPSHOT"
# JAVA应用程序端口号
SERVER_PORT=8200
# jar包存放路径
JAR_PATH='/software/basetrans'
# jar包名称
...
start() {
  ...
  if [ $? -eq "1" ]; then
    ...
    # jar服务启动脚本
    nohup java $JAVA_OPTS -jar $JAR_PATH/$JAR_NAME >$JAR_PATH/$LOG_FILE/run.log 2>&1 &
    echo $! > $JAR_PID
    echo "启动 $APP_NAME 成功 pid是 $! "
    sleep 15
    # 睡眠一会等启动完成后输出启动日志
    cat $JAR_PATH/$LOG_FILE/run.log
  fi
}
# 服务停止方法
stop() {
